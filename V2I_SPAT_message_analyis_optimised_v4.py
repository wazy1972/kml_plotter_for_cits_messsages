...
kml.save(path = 'C:/temp/comm-cam.kml')
...
kml.save(path = 'C:/temp/spat_timeadvice.kml')
...
kml.save(path = 'C:/temp/spat_phase_displayed.kml')
...
kml.save(path = 'C:/temp/spat_speedadvice_displayed.kml')

print("spat speed advice kml created") #break point

result1.to_excel("C:/temp/hmi_spat_advise.xlsx")
...
outputfile = 'C:/temp/xer_analysis.csv'
if os.path.isfile(outputfile) is True:
    ...
    out.write('Log_timestamp,moy,moytime,Stationid,Intersection_Name,SignalGroup,CurrentEvent,CurrentMinEndTime,CurrentMETCountdown,CurrentLikelyTime,CurrentLTCountdown,FutureEvents,xer\n')
    out.close()
...
with open(filename2) as csvfile:
    ...
    for row in readCSV:
        ...
        if messagetype == 'ETSI.SPATEM':
            ...
            moy = moy[0]
            current_year = datetime.datetime(int(year),1,1,0,0,0)
            moyutc = current_year + datetime.timedelta(0,(int(moy)*60))
            month = datetime.datetime.strftime(moyutc,'%m')
            day = datetime.datetime.strftime(moyutc,'%d')
            moyutc = datetime.datetime.strftime(moyutc,'%Y-%m-%d %H:%M:%S.%f')
            ...
            for i in MovementStates:
                ...
                if CurrentMeT != '':
                    ...
                    # assumed any other value tenths of seconds past the current hour. Current hour taken from the moy
                        ...
                        CurrentMeTtime = datetime.datetime.strftime(CurrentMeT,'%Y-%m-%d %H:%M:%S.%f')
                ...
                if CurrentLyt != '':
                    ...
                    # assumed any other value tenths of seconds past the current hour. Current hour taken from the moy
                        ...
                        CurrentLyttime = datetime.datetime.strftime(CurrentLyt,'%Y-%m-%d %H:%M:%S.%f')
                        # calculate the countdown
                ...
                newline = log_timestamp + ',' + moy + ',' + moyutc + ',' + stationid + ',' + intersectionID + ',' + Signal_Group + ',' + CurrentES + ',' +  CurrentMeTtime + ',' + CurrentMeTCD + ',' + CurrentLyttime + ',' + CurrentLytCD + ',' + FutureES + ',' + xer +'\n'
                out = open(outputfile, 'a')
                out.write(newline)
                out.close()
